...
function getRealOctave(value) {
  // Las teclas base van del 70 al 145
  // 70-100 = octava 0 (baja) - 31 notas
  // 101-131 = octava 1 (media) - 31 notas
  // 132-145 = octava 2 (alta parcial) - 14 notas
  if (value >= 70 && value <= 100) return 0;
  if (value >= 101 && value <= 131) return 1;
  if (value >= 132 && value <= 145) return 2;
  return 0;
}
...
function getFrequency(value) {
  ...
  const adjustedValue = value + (currentOctave * 31);
  if (adjustedValue >= 0 && adjustedValue < arrayT31.length) {
    return arrayT31[adjustedValue];
  }
  return null;
}
